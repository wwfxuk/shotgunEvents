...
def registerCallbacks(reg):
    ...
    reg.registerCallback(
        script_name, script_key, ticket_cc_alert, eventFilter, None,
    )
    ...
    attachments = [
        {
            ...
            "title": "You've been CC'd on Ticket #{}: {}".format(
    ...
    for ticket_cc in ticket_ccs:
        if ticket_cc == event.get("user"):
            ...
            group_users = sg.find_one(
                "Group", [["id", "is", ticket_cc["id"]]], ["users"]
            )
    ...
    for user in users:
        ...
        if slack_id:
            ...
            if slack_message["ok"]:
                ...
                logger.warning(
                    "New cc alert to {} failed to send with error: {}".format(
