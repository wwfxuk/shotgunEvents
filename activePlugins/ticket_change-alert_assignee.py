...
def ticket_assignment_alert(sg, logger, event, args):
    """
    ...
    ticket_assignees = event.get("meta", {}).get("added")

    # Bail if we don't have the info we need.
    if not event_project or not ticket_assignees:
        return
    ...
    for ticket_assignee in ticket_assignees:
        if ticket_assignee == event.get("user"):
            pass
        elif ticket_assignee["type"] == "HumanUser":
